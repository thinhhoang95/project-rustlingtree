...
from hailmary.scenario.models import FrozenPayload, ScenarioDefinition, freeze_payload, thaw_payload
...
class FlightDynamic:
    ...
    def __post_init__(self) -> None:
        ...
        origin = self.release_time_s if self.trajectory_origin_time_s is None else float(
            self.trajectory_origin_time_s
        )
        ...
        object.__setattr__(
            ...
            tuple((str(station_type), int(index)) for station_type, index in self.crossed_action_station_keys),
        )
        object.__setattr__(self, "crossed_resource_ids", tuple(str(item) for item in self.crossed_resource_ids))
        object.__setattr__(
            self,
            "predicted_resource_crossing_times",
            tuple((str(resource_id), float(time_s)) for resource_id, time_s in self.predicted_resource_crossing_times),
        )
        object.__setattr__(self, "action_history", tuple(str(item) for item in self.action_history))
        ...
        object.__setattr__(self, "intercept_gate_active", bool(self.intercept_gate_active))
        if len(set(self.crossed_action_station_indices)) != len(self.crossed_action_station_indices):
            raise ValueError("crossed action-station indices must be unique")
        if len(set(self.crossed_action_station_keys)) != len(self.crossed_action_station_keys):
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "crossed_action_station_keys": [list(item) for item in self.crossed_action_station_keys],
            "crossed_resource_ids": list(self.crossed_resource_ids),
            "predicted_resource_crossing_times": [list(item) for item in self.predicted_resource_crossing_times],
    ...
    def from_dict(cls, payload: Mapping[str, Any]) -> "FlightDynamic":
        return cls(
            ...
            crossed_action_station_indices=tuple(int(item) for item in payload.get("crossed_action_station_indices", [])),
            crossed_action_station_keys=tuple(
                (str(item[0]), int(item[1])) for item in payload.get("crossed_action_station_keys", [])
            ),
            crossed_resource_ids=tuple(str(item) for item in payload.get("crossed_resource_ids", [])),
            predicted_resource_crossing_times=tuple(
                (str(item[0]), float(item[1]))
                for item in payload.get("predicted_resource_crossing_times", [])
            ),
            action_history=tuple(str(item) for item in payload.get("action_history", [])),
...
class SimulationState:
    ...
    def __post_init__(self, _precomputed_dynamic_content_hash: str | None) -> None:
        ...
        object.__setattr__(self, "metrics", tuple((str(key), float(value)) for key, value in self.metrics))
        object.__setattr__(self, "action_log", tuple(freeze_payload(item) for item in self.action_log))
        object.__setattr__(self, "exogenous_state", freeze_payload(self.exogenous_state))
        object.__setattr__(
            self,
            "exogenous_event_log",
        ...
        if {flight.flight_id for flight in self.flights} != {flight.flight_id for flight in self.definition.flights}:
            raise ValueError("state flights must exactly match scenario-definition flights")
        resolved_hash = (
    ...
    def to_snapshot(self) -> dict[str, Any]:
        return {
            "schema_version": "hailmary.simulation-snapshot.v1",
    ...
    @classmethod
    def from_snapshot(cls, definition: ScenarioDefinition, payload: Mapping[str, Any]) -> "SimulationState":
        if str(payload.get("definition_hash", "")) != definition.definition_hash:
            raise ValueError("snapshot definition hash does not match supplied scenario definition")
        state = cls(
            definition=definition,
            state_id=str(payload["state_id"]),
            parent_state_id=(
                None if payload.get("parent_state_id") is None else str(payload.get("parent_state_id"))
            ),
            ...
            event_heap=tuple(ScheduledEvent.from_dict(item) for item in payload["event_heap"]),
            rng_state_json=_canonical_rng_state_json(payload["rng_state"]),
            metrics=tuple((str(item[0]), float(item[1])) for item in payload.get("metrics", [])),
            action_log=tuple(freeze_payload(item) for item in payload.get("action_log", [])),
        ...
        if expected_hash is not None and str(expected_hash) != state.dynamic_content_hash:
            raise ValueError("snapshot dynamic-content hash does not match reconstructed state")
        return state
