...
| `hailmary-visualize-route-graph` | `visualize_route_graph.py` | Pre-computed route graph and optional terminal-entry corpus | Local topology verifier GUI with canonical shared segments and queue gates |
| `hailmary-visualize-event-queue` | `visualize_event_queue.py` | Corpus, templates, optional route graph/rulebook, and selected demand window | Local queue, feature-provenance, action-rollout, and flight-radar verifier GUI |
